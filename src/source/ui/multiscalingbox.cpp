#include "../../header/multiscalingbox.h"
...
void MultiScalingBox::build_layout(QString mode){
    ...
    for(ProjectItem* item : items){
        if(true/*item_is_shown(item)*/){
            ...
            pb->setStyleSheet("background-color: #eeeeee;");
            pb->setParent(wbtn);
            wbtn->setMinimumHeight(60);
            wbtn->setMaximumHeight(60);
            wbtn->setMinimumWidth(80);
            wbtn->setMaximumWidth(80);
            columns[columns.size()-1]->addWidget(wbtn);
...
void MultiScalingBox::rescale_layout(){
    ...
    for(int i = 0; i < cells.size(); i++){
        for(int j = 0; j < cells[i].size(); j++){
            if(j == 0){
                cells[i][j]->setMinimumHeight(20);
                cells[i][j]->setMaximumHeight(20);
                continue;
            }
            cells[i][j]->setMinimumHeight(60);
            cells[i][j]->setMaximumHeight(60);
        }
    }
